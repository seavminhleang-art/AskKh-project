...
- Questions read `/posts`, detail and answers use their respective endpoints, and publishing uploads attachments through `/upload/upload-single` (`file`) or `/upload/upload-multiple` (`files`), then sends their returned `uri` values as `imageUrls` in JSON to `/posts`.
...
Post creation uses backend type `3` for both questions and replies, defined in `src/config/postTypes.js`; no post-type environment settings are needed. Reply statistics use `parentId`; legacy types 1/2 remain recognized. Community, trending, and personal post lists retain all returned posts, including records with a parent ID.
