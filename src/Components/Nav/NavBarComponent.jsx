...
import { Bell, ChevronDown, Menu, X, User } from "lucide-react";
...
function LanguageFlag({ isKhmer }) {
  ...
  return (
    ...
    >
      ...
      <path
        d="M5 14h22v1H5zm2-2h18v2H7zm2-3h14v3H9zm3-3h2v3h-2zm6 0h2v3h-2zm-3-2h2v8h-2zM8 10l2-2 2 2m8 0 2-2 2 2m-9-4 1-3 1 3"
        fill="#fff"
      />
    </svg>
  );
...
export default function Navbar({ notificationCount: propNotificationCount }) {
      ...
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-3 px-4 sm:gap-6 sm:px-6">
        ...
        <div className="hidden xl:flex items-center gap-2.5 shrink-0">
          ...
          >
            <Bell size={20} />
            {notificationCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-brand-secondary text-white text-[14px] font-bold flex items-center justify-center border-2 border-white dark:border-gray-900">
                {notificationCount}
              </span>
            )}
          </Link>
          ...
          {isAuthenticated ? (
            <Link
              to="/dashboard"
              className="h-10 px-4.5 inline-flex items-center gap-2 rounded-full border border-brand-primary bg-brand-primary text-white text-lg font-semibold no-underline transition-all duration-200 hover:bg-brand-secondary hover:border-brand-secondary hover:-translate-y-0.5 shadow-xs"
            >
              <User size={16} />
            ...
            <div className="flex items-center gap-2">
              <Link
                to="/login"
                className="h-10 px-4 inline-flex items-center rounded-full border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 text-lg font-semibold no-underline transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-800"
              >
                {t("login")}
              </Link>
              <Link
                to="/register"
                className="h-10 px-4.5 inline-flex items-center rounded-full border border-[rgba(255,255,255,0.35)] bg-brand-primary text-white text-lg font-semibold no-underline transition-all duration-200 hover:bg-brand-secondary hover:border-brand-secondary hover:-translate-y-0.5"
              >
                {t("getStarted")}
              </Link>
            </div>
      ...
      >
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Link
            ...
            className="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 transition-colors"
            aria-label="Notifications"
          >
            <Bell size={20} />
            {notificationCount > 0 && (
        ...
        {isAuthenticated ? (
          <Link
            to="/dashboard"
            onClick={() => setMobileOpen(false)}
            className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-brand-primary font-semibold text-white no-underline hover:bg-brand-secondary transition-colors"
          >
            <User size={18} />
            <span>{user?.displayName || user?.name || "Dashboard"}</span>
          </Link>
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full pt-1">
            <Link
              to="/login"
              onClick={() => setMobileOpen(false)}
              className="flex h-11 flex-1 items-center justify-center rounded-xl border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-gray-200 font-semibold no-underline hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
            >
              {t("login")}
            </Link>
            <Link
              to="/register"
              onClick={() => setMobileOpen(false)}
              className="flex h-11 flex-1 items-center justify-center rounded-xl bg-brand-primary font-semibold text-white no-underline hover:bg-brand-secondary transition-colors"
            >
              {t("getStarted")}
            </Link>
          </div>
