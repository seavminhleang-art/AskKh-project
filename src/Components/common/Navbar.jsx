...
export default function Navbar({ dashboardPath = "/dashboard" }) {
  ...
  const navLinkClasses = ({ isActive }) =>
    `px-3.5 py-2 text-[18px] font-medium transition-colors select-none rounded-lg ${
      isActive
        ...
        <div className="lg:hidden border-t border-[#E5E7EB] dark:border-slate-800 bg-white dark:bg-slate-950 px-6 py-5 space-y-4 shadow-xl animate-in fade-in slide-in-from-top-2 duration-200">
          <nav className="flex flex-col space-y-1">
            <NavLink
              ...
              className={({ isActive }) =>
                `px-4 py-3 rounded-xl text-[18px] font-medium transition-colors ${
                  isActive
          ...
          <div className="pt-3 border-t border-[#E5E7EB] dark:border-slate-800 flex flex-col gap-3">
            ...
            <button
              type="button"
              onClick={handleGetStarted}
              className="w-full h-11 rounded-xl bg-[#155EEF] hover:bg-[#124bbf] active:scale-[0.98] text-white text-[18px] font-semibold transition-all shadow-xs flex items-center justify-center cursor-pointer select-none"
            >
              Get Started
            </button>
          </div>
