  ...
  return (
    <div id={`section-${num}`} className="scroll-mt-24">
      <div className="flex items-start gap-4">
        <span className="shrink-0 w-9 h-9 rounded-full bg-brand-primary text-white text-lg font-bold flex items-center justify-center">
          {num}
        </span>
...
export default function PrivacyPolicy() {
  ...
  const sections = Object.entries(
    t("policyPage.sections", { returnObjects: true }),
  ).map(([num, data]) => ({
    num: parseInt(num),
    title: data.title,
  ...
  return (
    <div className="site-content bg-[var(--bg-main)] text-[var(--text-main)] antialiased transition-colors duration-300">
      <section
        className={`${darkMode ? "bg-zinc-900" : "bg-brand-primary-light"} transition-colors duration-300`}
      >
        <div className="max-w-4xl mx-auto px-6 py-16 text-center">
          <ScrollReveal animation="fadeInUp">
            <span className="text-brand-primary text-lg font-semibold uppercase tracking-wide">
              {t("policyPage.legalLabel")}
            </span>
            <h1 className="text-3xl md:text-5xl font-bold text-[var(--text-main)] mt-2">
              {t("policyPage.title")}
            </h1>
