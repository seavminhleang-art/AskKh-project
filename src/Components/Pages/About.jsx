...
function MemberCard({
  ...
  return (
    <div
      className={`rounded-3xl shadow-md overflow-hidden text-center pt-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 ${darkMode ? "bg-zinc-900 hover:shadow-brand-primary/10" : "bg-white hover:shadow-brand-primary/20"}`}
    >
      ...
      <div className="relative w-40 h-40 mx-auto my-5">
        ...
        >
          ...
          <span
            className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full shadow border ${darkMode ? "bg-zinc-900 border-zinc-800" : "bg-white border-slate-100"}`}
          />
        </div>
        {photo ? (
          <img
            src={photo}
            alt={t(name)}
            className={`absolute inset-2 w-[calc(100%-1rem)] h-[calc(100%-1rem)] rounded-full object-cover object-center border-4 shadow ${darkMode ? "border-zinc-800" : "border-white"}`}
          />
        ) : (
          <div
            className={`absolute inset-2 rounded-full border-4 shadow flex items-center justify-center ${darkMode ? "bg-zinc-800 border-zinc-900" : "bg-slate-200 border-white"}`}
          ></div>
        )}
      </div>
      ...
      <div className="mt-4">
        <span className="inline-flex items-center gap-1.5 text-base font-semibold text-brand-primary border border-brand-primary/50 rounded-full px-4 py-1.5">
          <PeopleIcon />
          {t(role).toUpperCase()}
        </span>
      </div>
          ...
          {featuresKeys.map((f, index) => (
            <ScrollReveal key={f.key} animation="fadeInUp" delay={index * 100}>
              <div className="bg-[var(--bg-card)] rounded-2xl p-6 text-center hover:shadow-md transition-all duration-300 hover:-translate-y-1">
                <div
                  className={`w-14 h-14 mx-auto rounded-full border-2 flex items-center justify-center mb-4 bg-[var(--bg-card)] ${f.color === "red" ? "border-brand-secondary/40 text-brand-secondary" : "border-brand-primary/50 text-brand-primary"}`}
                >
                  <svg
                    className="w-6 h-6"
                    fill="none"
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div className="order-1 md:order-2">
          <ScrollReveal animation="fadeInUp">
            ...
            <h2 className="text-4xl font-bold text-[var(--text-main)] mt-2 mb-4">
              {t("aboutPage.missionTitle")}
            </h2>
