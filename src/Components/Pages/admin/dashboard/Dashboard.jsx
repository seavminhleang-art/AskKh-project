    ...
    <article className="admin-stat-card">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-3xl font-bold tracking-tight text-white">{stat.value.toLocaleString()}</p>
        </div>
  ...
  return (
    <div className="admin-dashboard w-full">
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-400">Platform activity and moderation status at a glance.</p>
      </div>

      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Platform statistics">
        {mockDashboardStats.map((stat) => <StatCard key={stat.key} stat={stat} />)}
      </section>
        ...
        <article className="admin-panel p-5 sm:p-6 xl:col-span-2">
          ...
          <div className="mt-6 grid grid-cols-7 gap-2 sm:gap-3" aria-label="Weekly post activity">
            {activity.map((value, index) => (
              <div key={days[index]} className="flex h-44 flex-col justify-end gap-2">
      ...
      <section id="recent-posts" className="admin-panel mt-6 overflow-hidden">
        ...
        <div className="divide-y divide-white/10">
          {mockRecentPosts.map((post) => (
            <div key={post.id} className="admin-post-grid px-5 sm:px-6 py-4 text-sm">
              <p className="min-w-0 truncate font-semibold text-slate-100">{post.title}</p>
              <p className="text-slate-400">{post.author}</p>
              <p className="text-slate-400">{post.date}</p>
              <span className={`admin-status w-fit ${post.status === "Pending" ? "admin-status-pending" : "admin-status-published"}`}>{post.status}</span>
            </div>
          ))}
        </div>
      </section>
