...
export default function Tags() {
  ...
  const popular = useMemo(
    () =>
      [...(data || [])].sort((a, b) => b.usageCount - a.usageCount).slice(0, 4),
    [data],
  );
  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (t) => (
        <span className="font-medium text-brand-primary">#{t.name}</span>
      ),
    },
    ...
    { key: "usageCount", label: "Usage", sortable: true },
    {
      ...
      render: () => (
        <div className="flex items-center gap-1">
          ...
          <button className="p-1.5 text-gray-400 hover:text-brand-secondary transition-colors">
            <Trash2 size={15} />
          </button>
        </div>
      ),
  ...
  return (
    <div>
      <PageHeader
        title="Tags"
        description="Organize and curate content tags."
        actions={
