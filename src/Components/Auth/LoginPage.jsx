...
import { loginSuccess, setCredentials } from "../../features/auth/authSlice.js";
...
const translations = {
  km: {
    ...
    wrongCredentials: "អ៊ីមែល ឬពាក្យសម្ងាត់មិនត្រឹមត្រូវ។",

    disabled: "គណនីនេះត្រូវបានបិទដំណើរការ។",
    ...
    googleFailed: "ការចូលតាម Google បរាជ័យ។",

    githubFailed: "ការចូលតាម GitHub បរាជ័យ។",
    ...
    enterEmailBeforeReset: "សូមបញ្ចូលអ៊ីមែលជាមុនសិន។",

    resetFailed: "មិនអាចផ្ញើអ៊ីមែលកំណត់ពាក្យសម្ងាត់ឡើងវិញបានទេ។",
    ...
    successTitle: "ចូលគណនីជោគជ័យ",

    errorTitle: "មានបញ្ហា",

    infoTitle: "ព័ត៌មាន",

    resetTitle: "បានផ្ញើអ៊ីមែល",
  },
  ...
  en: {
    ...
    enterEmailBeforeReset: "Enter your email first.",
...
export default function LoginPage() {
  ...
  const showLoginSuccess = async (user) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.success(
        `${t.welcome}${res.displayName ? `, ${res.displayName}` : ""}!`,
        { toastId: "login-success" },
      );
  ...
  const handleOAuthError = (error, provider) => {
    ...
    if (error.code === "auth/popup-blocked") {
      notifyError(t.popupBlocked);

      return;
    }

    if (error.code === "auth/account-exists-with-different-credential") {
      notifyError(t.accountExists);

      return;
    }

    if (provider === "GitHub" && error.code === "auth/operation-not-allowed") {
      notifyError(t.githubDisabled);

      return;
    }

    notifyError(provider === "Google" ? t.googleFailed : t.githubFailed);
  };
  ...
  const handleForgotPassword = async () => {
    ...
    if (!email) {
      toast.info(t.enterEmailBeforeReset, { toastId: "reset-info" });
      return;
    }
    ...
    if (!result.success) {
      notifyError(result.error.issues[0]?.message || t.invalidEmail);
    ...
    try {
      ...
      toast.success(res?.message || t.resetSent, { toastId: "reset-success" });
    } catch (error) {
  ...
  return (
    <>
      ...
      {(isSubmitting || oauthLoading) && (
        <LoadingSpinner title="Signing in to NEXA" />
      )}

      <main className={`auth-page ${isKhmer ? "font-khmer" : "font-brand"}`}>
        <Link to="/" className="back-button" aria-label={t.back}>
          <ArrowLeft size={18} />
          <span>{t.back}</span>
        </Link>
          ...
          <div className="login-form-container">
            ...
            >
              <div className="form-group">
                ...
                {errors.email && (
                  <p
                    ...
                    role="alert"
                    className="mt-1.5 text-base font-medium text-red-500"
                  >
              ...
              <div className="login-options">
                ...
                >
                  {resetLoading ? t.resetSending : t.forgot}
                </button>
              </div>
              ...
              <div className="social-buttons">
                <GoogleComponent
                  label={t.google}
                  onSuccess={showOauthLoginSuccess}
                ...
                <GithubComponent
                  label={t.github}
                  onSuccess={showOauthLoginSuccess}
                  onError={(error) => handleOAuthError(error, "GitHub")}
                />
              </div>
