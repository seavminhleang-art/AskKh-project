...
const DetailView = ({ post, onBack, darkMode: propDarkMode }) => {
  ...
  const sortedComments = rowsOf(isAuthenticated ? query.currentData : post.comments).map(comment => ({ ...comment,
    author: { name: comment.userDisplayName || 'Community member', time: comment.creationDate ? new Date(comment.creationDate).toLocaleDateString() : '' },
  ...
  async function handleDeleteComment(id) {
    ...
    setError('');
    try { await deleteComment(id).unwrap(); }
    catch (error) { setError(errorMessage(error)); }
  }
  ...
  return (
    <div className={`min-w-0 flex-1 rounded-2xl p-6 space-y-6 transition-colors duration-300 ${
      ...
      <button
        onClick={onBack}
        className="flex items-center text-sm text-blue-500 hover:text-blue-600 font-semibold gap-1 hover:underline transition-colors"
      >
      ...
      <div className="flex flex-wrap gap-2">
        {post.tags?.map((tag, i) => (
          <span
            key={i}
            className={`px-2.5 py-1 text-sm rounded-full border font-medium ${
              darkMode
      ...
      {post.image && <img src={post.image} alt="Post attachment" className="max-h-96 max-w-full rounded-xl object-contain"/>}
      {post.codeSnippet && <section className="overflow-hidden rounded-xl border border-slate-300/30"><p className="px-4 py-2 text-sm opacity-60">{post.codeLanguage || 'Code'}</p><pre className="overflow-auto p-4 text-sm"><code>{post.codeSnippet}</code></pre></section>}
      {error && <p role="alert" className="text-sm text-red-500">{error}</p>}
      {query.isLoading && <p role="status">Loading comments…</p>}
      {query.isError && <p role="alert">Could not load comments. <button onClick={query.refetch}>Retry</button></p>}
      {!isAuthenticated && <p className="text-sm"><button className="text-blue-500" onClick={() => navigate('/login')}>Sign in</button> to load all comments and join the discussion.</p>}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <span className="grid h-6 w-6 place-items-center rounded-full bg-blue-100 text-blue-700">{comment.author.name.slice(0, 1)}</span>
                  <span className={`text-sm font-bold ${darkMode ? "text-slate-200" : "text-gray-800"}`}>
                    {comment.author.name}
                  </span>
                ...
                {comment.isOwnComment && (
                  <button
                    disabled={deletion.isLoading} onClick={() => handleDeleteComment(comment.id)}
