...
const DetailView = ({ post, onBack, darkMode: propDarkMode }) => {
  ...
  const { data: apiAnswers = [] } = useGetAnswersQuery(post?.id, {
    skip: !post?.id,
  });
  ...
  const commentsList =
    Array.isArray(apiComments) && apiComments.length > 0
      ? apiComments
  ...
  return (
    ...
    >
      ...
      <div>
        <h4
          className={`text-[14px] font-bold tracking-wider uppercase mb-1 ${
        ...
        <p
          className={`text-base leading-relaxed ${darkMode ? "text-slate-300" : "text-gray-600"}`}
        >
      ...
      >
        ...
        <div className="relative">
          <textarea
            ...
            className={`w-full rounded-xl p-3 pb-14 text-base border focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors ${
              darkMode
          ...
          <button
            ...
            className="disabled:opacity-50 disabled:cursor-not-allowed absolute bottom-3 right-3 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg text-base font-medium flex items-center gap-1 transition-colors"
          >
            <Send className="w-3 h-3" /> {t("detail.commentBtn")}
          </button>
        </div>
        <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg text-base font-medium text-blue-500 focus-within:ring-2 focus-within:ring-blue-500">
        ...
        <p className="text-base text-zinc-500">{t("detail.imageHint")}</p>
        {isReadingImage && (
        ...
        {commentImage && (
          <div className="relative w-fit">
            ...
            <button
              ...
              className="absolute right-2 top-2 rounded-full bg-zinc-900/80 p-1 text-white"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <div className="w-6 h-6 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center text-[14px]">
                    {(comment.userDisplayName || comment.author?.name || "S")
                      .charAt(0)
                      .toUpperCase()}
                  </div>
                  <span
              ...
              <div className="flex items-center space-x-2 pt-1">
                ...
                >
                  <Heart
                    className={`w-3.5 h-3.5 ${comment.isLiked ? "fill-rose-500 text-rose-500" : ""}`}
                  />
                  {comment.likes}
                </button>
              </div>
