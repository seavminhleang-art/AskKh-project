...
export default function AdminTable({
  columns, data = [], isLoading, isError, onRetry,
  pageSize = 6, emptyTitle = 'No records found', emptyDescription,
}) {
  ...
  if (isError) return <ErrorState message="Unable to load data" onRetry={onRetry} />
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {pageRows.map((row, i) => (
            <tr key={row.id ?? i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/70 transition-colors">
              {columns.map((col) => (
