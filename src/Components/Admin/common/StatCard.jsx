...
export default function StatCard({ icon: Icon, title, value, trend, direction, supportingText }) {
  const animated = useCountUp(typeof value === 'number' ? value : null)
  const displayValue = typeof value === 'number' ? animated.toLocaleString() : value
    ...
    <Card className="p-5 animate-fade-slide">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1.5 text-2xl font-semibold text-gray-900">{displayValue}</p>
        </div>
        {Icon && (
      ...
      {(trend !== undefined || supportingText) && (
        <div className="mt-3 flex items-center gap-1.5 text-xs">
          ...
          {supportingText && <span className="text-gray-400">{supportingText}</span>}
        </div>
      )}
    </Card>
  )
