...
export default function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-gray-100 h-16 flex items-center px-4 sm:px-6 gap-4">
      <button className="lg:hidden text-gray-500" onClick={() => dispatch(setSidebarMobileOpen(true))} aria-label="Open sidebar">
        <Menu size={22} />
      </button>
      ...
      <Dropdown
        trigger={<button className="flex items-center gap-2 cursor-pointer"><Avatar name={admin.name} size={32} /></button>}
      >
        <DropdownItem icon={User} onClick={() => navigate('/admin/settings')}>Profile</DropdownItem>
        <DropdownItem icon={Settings} onClick={() => navigate('/admin/settings')}>Settings</DropdownItem>
        <DropdownItem icon={LogOut} danger onClick={() => { dispatch(logout()); navigate('/login') }}>Logout</DropdownItem>
      </Dropdown>
    </header>
  )
}
