...
export default function UserAccount() {
  ...
  const displayName = isAuthenticated && (user?.displayName || user?.name)
    ? (user.displayName || user.name)
    : t('auth.guestUser');

  const email = isAuthenticated && user?.email
    ? user.email
    : t('auth.guestAccessDesc');
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="w-8 h-8 rounded-full bg-blue-100 text-[#102A56] dark:bg-blue-900/60 dark:text-blue-300 font-bold text-lg flex items-center justify-center shrink-0 border border-blue-200 dark:border-blue-800">
          {initials}
        </div>
          ...
          {isAuthenticated ? (
            <>
              <Link
                ...
                className="flex items-center gap-2 px-2.5 py-1.5 text-lg text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
              >
            ...
            <div className="p-2 text-center">
              ...
              <Link
                to="/profile"
                onClick={() => setIsOpen(false)}
                className="flex items-center justify-center gap-1.5 w-full py-1.5 text-lg font-semibold text-center text-white bg-[#102A56] hover:bg-[#102A56]/90 rounded-md transition-colors"
              >
