...
import { MapPin, Clock, ShieldCheck, Search, PlusCircle, Tag, Building2, X, CheckCircle2 } from 'lucide-react';
import {
  useGetReportsQuery,
...
export default function ItemFeedView({ onOpenReport, darkMode }) {
  ...
  // API Queries
  const itemTypeParam = activeTab === 'Lost Items' ? 'LOST' : activeTab === 'Found Items' ? 'FOUND' : undefined;
  const { data: apiReports = [], isLoading, isError, refetch } = useGetReportsQuery(itemTypeParam);
  ...
  const handleClaimSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Claim submitted successfully! Campus staff will review your proof.');
  ...
  const filteredItems = Array.isArray(apiReports) ? apiReports.filter((item) => {
    const matchesSearch =
      !searchQuery ||
      (item.title && item.title.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (item.description && item.description.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (item.locationLabel && item.locationLabel.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (item.freeTextLocation && item.freeTextLocation.toLowerCase().includes(searchQuery.toLowerCase()));

    const matchesCategory =
      selectedCategory === 'All Categories' ||
      item.categoryName === selectedCategory ||
      (item.categoryId && apiCategories.find((c) => c.id === item.categoryId)?.name === selectedCategory);

    return matchesSearch && matchesCategory;
  }) : [];

  if (sortBy === 'Oldest') {
    filteredItems.sort((a, b) => new Date(a.createdAt || a.itemDate || 0) - new Date(b.createdAt || b.itemDate || 0));
  } else {
    filteredItems.sort((a, b) => new Date(b.createdAt || b.itemDate || 0) - new Date(a.createdAt || a.itemDate || 0));
  }
      ...
      <main className="w-full">
        <div className={`flex border-b mb-6 gap-8 ${darkMode ? 'border-zinc-800' : 'border-gray-200'}`}>
          {[
            ...
            <button
              ...
              className={`pb-3 text-sm font-semibold relative transition-all duration-200 cursor-pointer hover:opacity-100 ${
                activeTab === tab.key
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-8">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              <option value="All Categories" className={darkMode ? 'bg-zinc-900 text-slate-100' : 'bg-white text-gray-800'}>
                All Categories
              </option>
              {apiCategories.map((cat, i) => (
                <option key={cat.id || i} value={cat.name} className={darkMode ? 'bg-zinc-900 text-slate-100' : 'bg-white text-gray-800'}>
                  {cat.name}
                </option>
              ))}
            </select>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3 space-y-4">
            {isLoading ? (
              ...
              <div className={`backdrop-blur-md rounded-3xl p-12 text-center text-sm font-medium border ${
                darkMode ? 'bg-zinc-900/90 border-zinc-800 text-slate-400' : 'bg-white/95 border-gray-100 text-gray-500'
              }`}>
                <p className="mb-3">Unable to load lost and found reports.</p>
                <button
                  onClick={() => refetch()}
                  className="px-4 py-2 rounded-xl bg-blue-600 text-white font-semibold text-xs hover:bg-blue-700 transition"
                >
                  Retry
                </button>
              </div>
              ...
              filteredItems.map((item) => {
                ...
                const location = item.locationLabel || item.freeTextLocation || 'Campus Area';
                ...
                const img = item.photoUrl || (isLost ? "src/assets/Website/kwfinwtieBa9DJNMHRxB63.jpg" : "src/assets/Website/OIP (1).webp");
                ...
                <motion.li
                  ...
                  className={`flex items-center justify-between text-xs cursor-pointer p-1.5 rounded-xl transition-colors ${
                    selectedCategory === 'All Categories'
                      ...
                      : darkMode ? 'text-slate-300 hover:bg-zinc-800/60' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <span className="pl-1">All Categories</span>
                  <span className={`pr-1 ${darkMode ? 'text-slate-500' : 'text-gray-400'}`}>{apiReports.length}</span>
                </motion.li>
              ...
              <ul className="space-y-2.5">
                {apiLocations.map((loc, idx) => (
                  <motion.li
                    ...
                    whileHover={{ x: 4 }}
                    transition={{ duration: 0.15 }}
                    className={`flex items-center justify-between text-xs p-1.5 rounded-xl transition-colors ${
                      darkMode ? 'text-slate-300' : 'text-gray-700'
                    }`}
                  >
                    <span className="pl-1">{[loc.building, loc.floor, loc.room].filter(Boolean).join(' - ') || 'Campus'}</span>
                  </motion.li>
                ))}
              </ul>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            <motion.div
              ...
              className={`w-full max-w-lg rounded-3xl p-6 shadow-2xl ${
                darkMode ? 'bg-zinc-900 text-white border border-zinc-800' : 'bg-white text-gray-900'
              }`}
            >
              ...
              <form onSubmit={handleClaimSubmit} className="mt-4 space-y-4">
                <div>
                  <p className="text-xs text-gray-500 dark:text-zinc-400 mb-1">Item Title</p>
                  <p className="text-sm font-semibold">{claimModalItem.title}</p>
                </div>

                <div>
                  <label className="block text-xs font-semibold mb-1">
                    Describe Unique Identifying Features <span className="text-red-500">*</span>
                  </label>
                  <p className="text-[11px] text-gray-500 dark:text-zinc-400 mb-2">
                    To verify ownership, describe private details not mentioned in the public listing (e.g., serial numbers, stickers, scratches, contents).
                  </p>
                  <textarea
                    required
                    rows="4"
                    ...
                    className={`w-full rounded-2xl p-3 text-xs border focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      darkMode
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setClaimModalItem(null)}
                    className="px-4 py-2 rounded-xl text-xs font-semibold border border-gray-200 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800 transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={isClaiming}
                    className="px-5 py-2 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white disabled:opacity-50 transition flex items-center gap-1.5"
                  >
