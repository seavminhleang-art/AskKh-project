...
export default function TrendingDiscussions({ darkMode }) {
  ...
  const fallbackDiscussions = [
    ...
    },
    {
      ...
      tagColors: [
        darkMode ? "bg-emerald-950/80 text-emerald-400" : "bg-emerald-50 text-emerald-600",
        darkMode ? "bg-zinc-800 text-slate-300" : "bg-gray-100 text-gray-600",
    ...
    },
    {
      ...
      tagColors: [
        darkMode ? "bg-amber-950/80 text-amber-400" : "bg-amber-50 text-amber-600",
        darkMode ? "bg-zinc-800 text-slate-300" : "bg-gray-100 text-gray-600",
  ...
  const discussions = apiPosts.length >= 3
    ? apiPosts.map((post, idx) => ({
        id: post.id,
        tags: post.tags?.length > 0 ? post.tags.map((t) => `#${t}`) : ["#Community", "#ISTAD"],
        tagColors: [
          darkMode ? "bg-blue-950/80 text-blue-400" : "bg-blue-50 text-blue-600",
          darkMode ? "bg-zinc-800 text-slate-300" : "bg-gray-100 text-gray-600",
          darkMode ? "bg-indigo-950/80 text-indigo-400" : "bg-indigo-50 text-indigo-600",
        ...
        title: post.title,
        excerpt: post.body?.length > 130 ? post.body.slice(0, 130) + "…" : post.body || "Join the conversation on AskKh.",
        author: post.ownerDisplayName || "Community Member",
        avatar: post.ownerProfileImage || fallbackDiscussions[idx % fallbackDiscussions.length].avatar,
    ...
    <section className="mt-28 mb-16 relative z-10 font-[family-name:var(--font-brand)]">
      <div className="text-center mb-12">
        ...
        <div className="flex justify-end max-w-7xl mx-auto px-2">
          <Link
            to="/community/qa"
            className="text-[var(--home-link-text)] font-semibold text-sm flex items-center gap-1 hover:underline cursor-pointer"
          >
            {t("discViewAll")} →
          </Link>
        </div>
            ...
            >
              ...
              <div className={`flex items-center justify-between pt-4 border-t text-sm ${
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className={`font-medium text-xs sm:text-sm truncate max-w-[110px] ${
