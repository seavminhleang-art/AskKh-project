...
const themes = [{ value: 'light', label: 'Light', icon: Sun }, { value: 'dark', label: 'Dark', icon: Moon }, { value: 'system', label: 'System', icon: Monitor }]
...
export default function AppearancePanel() {
  ...
  return (
    <Card className="p-6">
      ...
      <p className="text-sm text-gray-500 mb-5">Personalize how the admin dashboard looks for you. Saved to this device.</p>

      <p className="text-sm font-medium text-gray-700 mb-2">Theme</p>
      <div className="flex gap-2 mb-5">
        {themes.map(({ value, label, icon: Icon }) => (
          <button key={value} onClick={() => dispatch(setTheme(value))}
      ...
      <p className="text-sm font-medium text-gray-700 mb-2">Sidebar</p>
      <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg mb-5">
        <span className="text-sm text-gray-600">Collapsed by default</span>
        <button
          onClick={() => dispatch(toggleSidebar())}
          className={`w-10 h-5.5 rounded-full transition-colors relative ${sidebarCollapsed ? 'bg-brand-primary' : 'bg-gray-200'}`}
          style={{ height: 22 }}
        >
          <span className={`absolute top-0.5 w-4.5 h-4.5 rounded-full bg-white transition-transform ${sidebarCollapsed ? 'translate-x-5' : 'translate-x-0.5'}`} style={{ width: 18, height: 18 }} />
        </button>
      </div>

      <p className="text-sm font-medium text-gray-700 mb-2">Table Density</p>
      <div className="flex gap-2">
        {densities.map((d) => (
          <button key={d} onClick={() => dispatch(setDensity(d))}
