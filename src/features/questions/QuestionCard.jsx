...
export default function QuestionCard({ question }) {
  ...
  const authorName =
    question.ownerDisplayName || question.author?.name || "ISTAD Scholar";
  const authorAvatar =
  ...
  const isAccepted =
    question.answers?.some((a) => a.isAccepted) || question.isResolved;
  const views = question.viewCount ?? question.views ?? 1;
  const createdDate =
    question.creationDate || question.createdAt || new Date().toISOString();
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0 space-y-2.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isAccepted && (
              <span className="inline-flex items-center gap-1 text-[16px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800">
          ...
          >
            <h3 className="text-base sm:text-base font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-snug">
              {question.title}
            </h3>
          </Link>
