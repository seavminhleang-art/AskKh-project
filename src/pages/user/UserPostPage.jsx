...
export default function UserPostPage({ kind }) {
  ...
  const { data: categories = [] } = useGetCategoriesQuery(undefined, { skip: isQuestion });
  const { data: locations = [] } = useGetLocationsQuery(undefined, { skip: isQuestion });
  ...
  const submit = async (event) => {
    ...
    if (isQuestion) {
      ...
      if (form.description.trim().length < 20) {
        setFormError('Question description must be at least 20 characters long.');
        return;
      }
      ...
      try {
        ...
        const msg = err?.data?.message || err?.error || 'Failed to create question.';
        setFormError(msg);
        toast.error(msg);
      }
    } else {
      ...
      try {
        ...
        if (selectedFile) {
          ...
          const uri = uploadRes?.uri || uploadRes?.url || (typeof uploadRes === 'string' ? uploadRes : null);
        ...
        const msg = err?.data?.message || err?.error || 'Failed to submit report.';
        setFormError(msg);
        toast.error(msg);
      }
    }
  ...
  return (
    <section className="mx-auto max-w-3xl">
      ...
      <form onSubmit={submit} className="mt-6 space-y-5 rounded-3xl bg-white p-6 dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800">
        {!isQuestion && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
              ...
              <select
                ...
                value={form.type}
                onChange={update}
                className="mt-2 w-full rounded-xl border border-slate-200 bg-white p-3 text-sm dark:border-slate-700 dark:bg-slate-800 text-slate-900 dark:text-white"
              >
            ...
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
              ...
              >
                {Array.isArray(categories) && categories.map((c) => (
                  <option key={c.id} value={c.id}>{c.name || c.categoryName || `Category #${c.id}`}</option>
                ))}
            ...
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
              ...
              >
                {Array.isArray(locations) && locations.map((l) => (
                  <option key={l.id} value={l.id}>{l.name || l.locationName || `Location #${l.id}`}</option>
                ))}
        ...
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
          ...
          <input
            ...
            value={form.title}
            onChange={update}
            placeholder={isQuestion ? 'e.g. How do I configure Redux Toolkit with React 19?' : 'e.g. Blue student ID card'}
            className="mt-2 w-full rounded-xl border border-slate-200 bg-white p-3 text-sm dark:border-slate-700 dark:bg-slate-800 text-slate-900 dark:text-white"
          />
        </label>
        ...
        {!isQuestion && (
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
            Photo <span className="font-normal text-slate-500">(optional, under 5 MB)</span>
            <input
              type="file"
              accept="image/*"
              onChange={selectImage}
              className="mt-2 block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
            />
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={isLoading}
            className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2"
          >
