...
const kinds = {
  Reports: { icon: PackageCheck, tone: "orange", verb: "Report an item" },
...
function ProgressChart({ data, available, w }) {
  ...
  return <section className="activity-panel activity-chart"><div className="activity-panel-title"><h2>{w("Monthly Progress")}</h2><div className="activity-chart-legend"><span>{w("This")}</span><span>{w("Last")}</span></div></div>
    ...
    <small>{w("Posts, answers, comments, and reports by week of month.")}</small>
  </section>;
}
export default function ActivityPage() {
  const { w, locale } = useWorkspaceTranslation();
  const reportsQuery = useWorkspaceDataQuery({ resource: "my-reports" });
  const reports = rows(reportsQuery.data);
  ...
  const ready = postsQuery.isSuccess && profileQuery.isSuccess && reportsQuery.isSuccess;
  ...
  const stats = [["Posts Created", countFor("Posts", postsQuery), FileText, "blue", "Posts"], ["Items Found", reportsQuery.isSuccess ? reports.filter(item => item.itemType?.toUpperCase() === "FOUND").length : reportsQuery.isError ? "—" : "…", PackageCheck, "green", "Reports"], ["Items Lost", reportsQuery.isSuccess ? reports.filter(item => item.itemType?.toUpperCase() === "LOST").length : reportsQuery.isError ? "—" : "…", CircleHelp, "orange", "Reports"], ["Comments", countFor("Comments", profileQuery), MessageSquare, "violet", "Comments"], ["Claims Submitted", "—", ShieldCheck, "purple"]];
  const visible = events.filter(event => filter === "All Activity" || event.kind === filter);
  const unsupported = ["Claims", "Matches"].includes(filter);
  const activeQuery = filter === "Reports" ? reportsQuery : filter === "Comments" ? profileQuery : postsQuery;
  ...
  return <div className="uw-page activity-page">
    ...
    <div className="activity-stats">{stats.map(([label, count, Icon, tone, kind]) => {
      const matchesStat = event => event.kind === kind && (label === "Items Found" ? event.itemType === "FOUND" : label === "Items Lost" ? event.itemType === "LOST" : true);
      const current = metrics.current.filter(matchesStat).length;
      const previous = metrics.previous.filter(matchesStat).length;
    ...
    <div className="activity-layout"><section className="activity-panel activity-feed">
      ...
      {filter === "All Activity" && reportsQuery.isError && <QueryState query={reportsQuery} />}
